/*
 * Semantic (Jaccard) clustering + heuristic title generation for conversation
 * branch labels.
 *
 * Ported ~1:1 from local-lmcanvas (MIT License, Copyright (c) 2026 Max Lee):
 *   src/renderer/src/lib/groupClustering.ts
 * (which itself mirrors avera's group-pairs.ts). Only adaptations: the import
 * points at this app's `./groupSummary` types and lmcanvas's `NodeId` collapses
 * to a plain `string`. The clustering convention is otherwise identical so the
 * grouping matches the donor exactly.
 */
